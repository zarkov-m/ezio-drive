...
func usage() {
	...
	fmt.Println("  go run . whoami --profile mitko")
	fmt.Println("  go run . list --query \"isRead eq false\" --expect-user m.zarkov@hksglobal.group")
	fmt.Println("  go run . list --profile mitko --expect-user m.zarkov@hksglobal.group --top 10")
	fmt.Println("  go run . read --id <message-id> --expect-user m.zarkov@hksglobal.group")
...
func cmdSend(args []string) {
	...
	profile := fs.String("profile", "", "Token cache profile (e.g. mitko). Uses OUTLOOK_PROFILE when omitted")
